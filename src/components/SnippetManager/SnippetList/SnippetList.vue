...
<template>
  <div class="wrapper flex flex-col space-y-2" >
    <SnippetCard v-for="snippet in snippets" :key="snippet.shortcut" :name="snippet.name" :text="snippet.text" :shortcut="snippet.shortcut" @delete-snippet="emit('delete-snippet', $event)"/>
  </div>
</template>
